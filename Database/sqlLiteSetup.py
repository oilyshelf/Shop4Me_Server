...
curs.execute(
    "CREATE TABLE user (user_id INT AUTO_INCREMENT , last_name VARCHAR(255), first_name VARCHAR(255), postcode Int(255), street VARCHAR(255), house_number INT(255), Primary key (user_id), Foreign key (postcode) References place(postcode) )")

curs.execute(
    "CREATE TABLE item (item_id INT AUTO_INCREMENT , name VARCHAR(255), description VARCHAR(255), price INT(255), primary key (item_id))")

curs.execute(
    "CREATE TABLE errand (errand_id INT AUTO_INCREMENT , postcode VARCHAR(255), email VARCHAR(255), phone_number VARCHAR(255), notice VARCHAR(255), primary key (errand_id))")

curs.execute("CREATE TABLE items_in_errand (item_id INT(255), errand_id INT(255), primary key(item_id,errand_id) Foreign key (item_id) references item(item_id), foreign key (errand_id) references errand(errand_id))")
