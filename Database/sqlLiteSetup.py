...
curs.execute(
    "CREATE TABLE user (user_id INT AUTO_INCREMENT , email VARCHAR(255), last_name VARCHAR(255), first_name VARCHAR(255), postcode VARCHAR(255), street VARCHAR(255), house_number VARCHAR(255), password VARCHAR(255), Primary key (user_id), Foreign key (postcode) References place(postcode) )")
...
curs.execute(
    "CREATE TABLE errand (errand_id INT AUTO_INCREMENT , status INT default 0, startDate DATE , endDate DATE, postcode VARCHAR(255), email VARCHAR(255), phone_number VARCHAR(255), notice VARCHAR(255), primary key (errand_id))")
...
with open("itemlist.csv", newline='', encoding='utf-8-sig') as csvfile1:
